...
Mobile tracking surveys resulted in `r nrow(xs_use)` observations of `r nrow(use_unique)` radio tagged Chinook.  Of those `r nrow(use_unique)` tags, `r nrow(use_recovered)` were physically recovered from the river (one confirmed mortality). To further refine our analysis to include only those locations deemed utilized by fish, we used a decision tree (Figure \@ref(fig:tree-fig)) developed by combining information from @Holleman2022 and professional judgement. This resulted in `r nrow(use_selected)` unique habitat locations utilized by `r nrow(unique_tag)` fish.

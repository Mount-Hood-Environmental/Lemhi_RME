...
Degradation of tributary habitat has been implicated as a major factor contributing to declines of Pacific salmon *Oncorhynchus* spp. and has shown to reduce salmonid carrying capacity of freshwater ecosystems [@Beechie2013]. Tributary habitat rehabilitation efforts have been employed for decades to stem population declines [@Bernhardt2005]. However, habitat use and preference for juvenile salmonids to inform those rehabilitation actions, particularly during winter months, are not well documented [@Huusko2007]. 
...
load(here("analysis/data/derived_data/d_v_avail_sin.rda")) # d_v_avail_sin
...
comp_summ <- comp_df %>%
  ...
  mutate(ks = map(data,
                  .f = function(x) {
                    ks.test(x = x$raster_val[x$source == "all"],
                            y = x$raster_val[x$source == "sampled"],
                            alternative = "two.sided")
                  }),
         ks_pvalue = map_dbl(ks,
                             "p.value")) %>%
  # Mann-Whitney U test
  mutate(mwu = map(data,
                   .f = function(x) {
                     wilcox.test(raster_val ~ source,
                                 data = x,
                                 paired = F,
                                 digits.rank = 5,
                                 alternative = "two.sided")
                   }),
         mwu_pvalue = map_dbl(mwu,
                              "p.value")) #%>%
  # # different version of Kolmogorov–Smirnov test 
  # mutate(ks2 = map(data,
  #                .f = function(x) {
  #                  ks_test(a = x$raster_val[x$source == "all"],
  #                          b = x$raster_val[x$source == "sampled"])
  #                }),
  #      ks2_pvalue = map_dbl(ks2,
  #                           "P-Value")) %>%
  # # Kuiper test
  # mutate(kp = map(data,
  #                 .f = function(x) {
  #                   kuiper_test(a = x$raster_val[x$source == "all"],
  #                               b = x$raster_val[x$source == "sampled"])
  #                 }),
  #        kp_pvalue = map_dbl(kp,
  #                            "P-Value")) %>%
  # # Anderson-Darling test
  # mutate(ad = map(data,
  #                 .f = function(x) {
  #                   ad_test(a = x$raster_val[x$source == "all"],
  #                           b = x$raster_val[x$source == "sampled"])
  #                 }),
  #        ad_pvalue = map_dbl(ad,
  #                            "P-Value")) %>%
  # # Cramer-Von Mises test 
  # mutate(cvm = map(data,
  #                  .f = function(x) {
  #                    cvm_test(a = x$raster_val[x$source == "all"],
  #                             b = x$raster_val[x$source == "sampled"])
  #                  }),
  #        cvm_pvalue = map_dbl(cvm,
  #                             "P-Value")) %>%
  # # Wass test
  # mutate(wass = map(data,
  #                   .f = function(x) {
  #                     wass_test(a = x$raster_val[x$source == "all"],
  #                               b = x$raster_val[x$source == "sampled"])
  #                   }),
  #        wass_pvalue = map_dbl(wass,
  #                              "P-Value")) %>%
  # # DTS test
  # mutate(dts = map(data,
  #                  .f = function(x) {
  #                    dts_test(a = x$raster_val[x$source == "all"],
  #                             b = x$raster_val[x$source == "sampled"])
  #                  }),
  #        dts_pvalue = map_dbl(dts,
  #                             "P-Value"))
```
...
We estimated the proportion of the total available habitat in the lower Lemhi River that fell into each of the sinuosity categories (Table \@ref(tab:sin-table)), which was compared to the proportion of each category that Chinook salmon presmolts used. We used an exact multinomial test as well as a likelihood ratio, or G, test to compare the proportions of sinuosity classes available to those where fish were observed using them.
...
Comparisons of the distributions of depths and velocities estimated from 1) the randomly placed transects used to estimate habitat availability, and 2) the entirety of the rasters derived from 2D numerical models were favorable (Figure \@ref(fig:dist-fig) and Figure \@ref(fig:dist-cdf-fig)). Although the KS test and Mann-Whitney U test were sometimes statistically significant, the distributions and empirical cumulative distribution functions of depths and velocities were very similar among all sinuosity categories (Figure \@ref(fig:dist-fig) and Figure \@ref(fig:dist-cdf-fig)). These findings suggest that the habitat availability data collected during August 2019 were sufficient to capture the available habitat throughout the lower Lemhi River.
...
comp_summ %>%
  ...
  kbl(digits = 4,
      ...
      col.names = c("Metric",
                    ...
                    "KS p-value",
                    "MWU p-value"),
      caption = "P-values for the Kolmogorov–Smirnov and Mann-Whitney U tests, comparing all values of depth or velocity within a sinuosity class with the values at sampled transects.") %>%
  kable_styling()
```
...
```{r dist-cdf-fig, fig.cap = "Empirical cumulative distribution plots of depth and velocity, colored by whether taken from the entire raster (all) or the sampled transects (sampled), and faceted by sinuosity category."}
comp_df %>%
  mutate(source = recode(source,
                         `sampled` = "Sampled",
                         `all` = "All")) %>%
...
sin_test <- xs_use %>%
  ...
  mutate(sin_test = map(obs,
                        .f = purrr::quietly(function(x) {
                          EMT::multinomial.test(observed = x,
                                                prob = sin_all$Percentage,
                                                useChisq = F,
                                                MonteCarlo = T,
                                                ntrial = 1e5)
                        })),
         mn_p_value = map_dbl(sin_test,
                           .f = function(x) {
                             x$result$p.value
                           })) %>%
  mutate(g_test = map(obs,
                      .f = purrr::quietly(function(x) {
                        DescTools::GTest(x = x,
                                         p = sin_all$Percentage,
                                         correct = "williams")
                      })),
         g_p_value = map_dbl(g_test,
                             .f = function(x) x$result$p.value))

```

Sinuosity categories selected by radio-tagged Chinook salmon presmolts were similar to the proportions of available habitat (Figure \@ref(fig:sin-use)), suggesting habitat use by sinuosity category was random. The multinomial and G goodness of fit tests returned non-significant p-values (`r round(sin_test$mn_p_value, 3)` and `r round(sin_test$g_p_value, 3)`). In other words, presmolts were *not* observed using "high" sinuosity reaches of the lower Lemhi River at a higher rate than was available, as we might expect. This result is further considered below in the [Discussion](#discussion) section.
...
Distributions between available stream depths and depths used by Chinook salmon presmolts were similar (Figure \@ref(fig:depth-fig)). Although the "used" mean depth overall was significantly lower than the available mean (p = 0.016; Table \@ref(tab:depth-tab)), when broken out by sinuosity class there was not a significant difference (Table \@ref(tab:depth-sin-tab). The former significance could (at least partially) be explained by a potential sampling bias considered in the [Discussion](#discussion) section.
...
d_v_avail = d_v_avail_sin %>%
  as_tibble() %>%
  add_column(Habitat = "Available") %>%
  select(Habitat, sin_class, 
         metric, value)

use_d_v <- use_selected %>%
  select(sin_class = sin_cat,
         depth_m, mean_column_velocity) %>%
  rename("depth" = depth_m, 
         "velocity" = mean_column_velocity) %>%
  pivot_longer(depth:velocity,
               names_to = "metric") %>%
  na.omit(value) %>%
  add_column(Habitat = "Used") %>% 
  bind_rows(d_v_avail) %>%
  mutate(across(sin_class,
                factor,
                levels = c("Low",
                           "Med",
                           "High")))
...
depth_tab = t.test(value ~ Habitat, 
                   data = stat_depth) %>%
    tidy() %>%
    select(estimate1,
           estimate2,
  ...
  kable(digits = 3,
        caption = "Estimated mean depth for available and used habitat and Welch t-test p-value.") %>%
...
stat_depth %>%
  ...
  mutate(t_test = map(data,
                      .f = function(x) {
                        t.test(value ~ Habitat, 
                               data = x)
                      })) %>%
  mutate(est = map(t_test,
                   "estimate"),
         est = map(est,
                   .f = function(x) {
                     x %>%
                       ...
                       mutate(across(group,
                                     recode,
                                     "mean in group Available" = "Mean Available Depth (m)",
                                     "mean in group Used" = "Mean Used Depth (m)")) %>%
                       pivot_wider(names_from = "group",
                                   values_from = "value")
                   }),
         p_value = map_dbl(t_test,
                           "p.value")) %>%
  unnest(est) %>%
  select(-data,
         -t_test) %>%
  rename(`Sinuosity Class` = sin_class,
         `Welch t-test p-value` = p_value) %>%
  kable(booktabs = T,
        digits = 3,
        caption = "Estimated mean depth for available and used habitat and Welch t-test p-value, by sinuosity category.") %>%
  kable_styling(position = "center", 
                full_width = F)
```


Mean stream velocities used by Chinook salmon presmolts were significantly lower (p = 0) than those available both throughout the lower Lemhi River and by sinuousity class (Figure \@ref(fig:depth-fig); Table \@ref(tab:vel-tab); Table \@ref(tab:vel-sin-tab)), suggesting that presmolts were selecting for slower water habitats during late fall and early winter months.
...
stat_vel %>%
         ...
         est = map(est,
                   .f = function(x) {
                     x %>%
                       ...
                       mutate(across(group,
                                     recode,
                                     "mean in group Available" = "Mean Available Velocity (m/s)",
                                     "mean in group Used" = "Mean Used Velocity (m/s)")) %>%
                       pivot_wider(names_from = "group",
                                   values_from = "value")
                   }),
         p_value = map_dbl(t_test,
  ...
  kable(booktabs = T,
        digits = 3,
        caption = "Estimated mean velocity for available and used habitat and Welch t-test p-value, by sinuosity category.") %>%
  kable_styling(position = "center", 
                full_width = F)
```
...
cu_tab = cu_prop_test %>%
  ...
  kable(digits = 3,
        caption = "P-values of multinomial and G-tests for differences in channel unit type proportions between available and selected habitat.") %>%
  kable_styling(position = "center", 
                full_width = F)
cu_tab
...
We observed little difference between the proportion of locations where concealment substrate was available and locations used by Chinook salmon presmolts with concealment habitat, for both the medium and high sinuosity categories (Figure \@ref(fig:conceal-fig)). While it appears differently (due to sample size), we found no significant difference in concealment  for high sinuosity (Table \@ref(tab:conceal-tab)). There was a significant difference (p = 0.0061) within the low sinuosity category (Table \@ref(tab:conceal-tab)); however; Figure \@ref(fig:conceal-fig) indicates that in low sinuosity reaches, fish were more likely to select habitat that does not contain substrate concealment, contrary to our suspicions.
...
In this study, we identified that radio-tagged Chinook salmon presmolts selected for slower stream velocities that were typically available in the lower Lemhi River; and further, selected for slow-water channel unit types, including pools and off-channel areas (Figure \@ref(fig:cu-fig)), at a higher rate than those types were available. Chinook presmolts appeared to similarly select for small side channels (Figure \@ref(fig:cu-fig)). Additionally, we identified that presmolts selected for locations with adjacent cover (of any type), regardless of the reach-scale sinuosity of the river. We did not identify a pattern of selection (or not) for concealment habitat. In the following, we further discuss these results including potential limitations that should be considered during interpretation.
